	   ...
	   $this->message[] = 'Removed Pending Invitation for Past Event';
        ...
        public function _processFileStorage($updateNum = 5)
        {
            ...
            if($fileStorage != 'file')
            {
                ...
                $sql = 'SELECT * FROM ' . $db->nameQuote('#__community_files')
				.' WHERE ' . $db->nameQuote('storage') .'!=' . $db->Quote($fileStorage)
				.' ORDER BY rand() limit '.$updateNum;
		...
		if(!$result)
		{
			...
			return;
                ...
                foreach($result as $row)
                {
                    ...
                    {
                        // File exist on remote storage, move it locally first
                        $tempFilename = $jconfig->getValue('tmp_path'). DS . md5($row->filepath);
