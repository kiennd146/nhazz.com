...
class CommunityViewFrontpage extends CommunityView
{
	public function display()
	{
		...
		if($config->get('eventfrontpagelist')){
		    $eventListSettings='showLatestEvents';
		} else {
		    $eventListSettings='showFeaturedEvents';
		}

		if($config->get('groupfrontpagelist')){
		    $groupListSettings='showLatestGroups';
		} else {
		    $groupListSettings='showFeaturedGroups';
		}
		...
		$latestGroupsData   =	$this->_cachedCall($groupListSettings, array( $config->get('frontpagegroups') ), '', array( COMMUNITY_CACHE_TAG_FRONTPAGE ) );
		...
		$latestEventsData   =	$this->_cachedCall($eventListSettings, array($config->get('frontpage_events_limit')), '', array( COMMUNITY_CACHE_TAG_FRONTPAGE ) );
