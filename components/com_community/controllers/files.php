...
class CommunityFilesController extends CommunityBaseController
{
	...
	public function multiUpload()
	{
		...
		{
			$json = array('msg'=>'Error:Failed uploaded file.Daily limit reached');
		...
		foreach($files as $_file)
		{
			...
			{
				$json = array('msg'=>'Error:File type not valid');
				echo json_encode($json);
				exit;
			}
			...
			{
				JFolder::create( JPATH_ROOT . DS . 'images' . DS . 'files' . DS . $type . DS . $parentTable->id , (int) octdec( $config->get('folderpermissionsphoto') ) );
