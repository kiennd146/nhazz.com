...
class CommunityModelActivities extends JCCModel
{
	 ...
	 */	 	
	public function getActivities($userid='', $friends='', $afterDate = null, $maxEntries=20 , $respectPrivacy = true , $exclusions = null , $displayArchived = false,$actid=null ){
		$db	 = &$this->getDBO();
		$my  = CFactory::getUser();
		...
		*/
		...
		if(!empty($groupIds)){
			$andWhere[] = '( (a.' . $db->nameQuote('group_access') .'=' . $db->Quote(0).')'
					 ...
					 .' OR (a.' . $db->nameQuote('groupid') .'=' . $db->Quote(0).'))';
		} else
		{
		...
		if(!empty($groupIds)){
			$andWhere[] = '( (a.' . $db->nameQuote('event_access') .'=' . $db->Quote(0).')'
					 ...
					 .' OR (a.' . $db->nameQuote('eventid') .'=' . $db->Quote(0).') )';
		}
		else 
		{
