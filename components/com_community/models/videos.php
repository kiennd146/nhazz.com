	...
	public function getInviteListByName($namePrefix ,$userid, $cid, $limitstart = 0, $limit = 8){
		...
		if($video->groupid){
			...
			$joinQuery	= ' INNER JOIN ' . $db->nameQuote( '#__users' ) . ' AS b '
					...
					. ' AND a.'.$db->nameQuote('memberid').'!=' . $db->Quote( $my->id )
					. ' AND a.'.$db->nameQuote('groupid').'=' . $db->Quote( $video->groupid )
					. ' WHERE NOT EXISTS (SELECT e.'.$db->nameQuote('userid') . ' as id'
										. ' FROM '.$db->nameQuote('#__community_videos_tag') . ' AS e  ' 
										. ' WHERE e.'.$db->nameQuote('videoid').' = '.$db->Quote($cid)
