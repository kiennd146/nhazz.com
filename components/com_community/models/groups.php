		...
		if( !is_null($categoryId) && $categoryId != 0 )
		{
                    if (is_array($categoryId)) {
                        if (count($categoryId) > 0) {
                            ...
                            $extraSQL .= ' AND a.' . $db->nameQuote('categoryid'). ' IN(' . $categoryIds . ')';
                        }
                    } else {
...
// 		}
	...
	public function getGroupDiscussionLastActive( $userid ){
		...
		$query = "SELECT contentid FROM ".$db->nameQuote('#__community_wall')." WHERE type='discussions' AND post_by=".$db->Quote($userid);
		$db->setQuery( $query );
		
		$discussionIds = $db->loadAssocList();
		$discussionIds = JArrayHelper::getColumn($db->loadAssocList(),'contentid');
		...
		if(count($discussionIds) > 0){
			...
			$query = "SELECT g.id AS discussionid, w.post_by, w.comment, w.date, g.groupid, g.title, gr.name AS group_name 
						FROM ".$db->nameQuote('#__community_wall')." w, ".$db->nameQuote('#__community_groups_discuss')." 
						g, ".$db->nameQuote('#__community_groups')." gr WHERE w.contentid = g.id AND w.type='discussions' 
	...
	public function getGroupAnnouncementUpdate( $userid, $limit = 3 ){
		...
		if(count($userGroups) > 0){
			...
			$query	= 'SELECT b.*, g.name AS group_name FROM ' . $db->nameQuote('#__community_groups_bulletins')
					. ' AS b, '.$db->nameQuote('#__community_groups').' AS g WHERE b.groupid IN (' . $groups_id .' ) AND g.id=b.groupid ORDER BY b.id DESC '.$extraSQL;
	...
	public function getGroupLatestAlbumUpdate( $userId, $limit = 3 ){
		...
		if(count($userGroups) > 0){
			$groups_id = implode(',',$userGroups);
			$db	=& $this->getDBO();
			$query	= 'SELECT * FROM (SELECT a.id as photo_id, a.creator as creator_id, b.name as album_name,b.id as album_id, b.groupid FROM ' . $db->nameQuote('#__community_photos') . ' AS a '
					. ', ' . $db->nameQuote('#__community_photos_albums') . ' AS b '
	...
	public function getGroupUpcomingEvents($userId, $limit = 3){
		...
		if(count($userGroups) > 0){
			...
			$query = "SELECT * FROM ". $db->nameQuote('#__community_events') ." WHERE catid IN (" . $groups_id ." ) AND 
					((parent = 0 AND  (".$db->nameQuote('repeat')." IS NULL OR  ".$db->nameQuote('repeat')." =  '')) 
					OR (parent > 0 AND  ".$db->nameQuote('repeat')." IS NOT NULL)) AND ".$db->nameQuote('published')." = 1 AND  
					".$db->nameQuote('type')." =  'group' AND ". $db->nameQuote('enddate')." >= " . $db->Quote( $now->toMySQL() )
					." ORDER by startdate ASC ".$extraSQL;
			$db->setQuery( $query );
			
			$eventsDetails = $db->loadAssocList();
		}
	...
	public function getGroupVideosUpdate( $userId, $limit = 3 ){
		...
		if(count($userGroups) > 0){
			...
			$query	= "SELECT * FROM " . $db->nameQuote('#__community_videos') 
					  . " WHERE ".$db->nameQuote('creator_type')."='group' AND "
					  . $db->nameQuote('groupid')." IN (" . $groups_id .") ORDER BY created DESC ".$extraSQL;
			$db->setQuery( $query );
