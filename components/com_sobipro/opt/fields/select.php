	...
	public function searchString( $data, $section )
	{
		...
		try {
			...
			$fids = $db->loadResultArray();
                        if(!count($fids)) {
                            //$temp = "REGEXP:[[:<:]]{$search}[[:>:]]";
                            $temp = str_replace("REGEXP:[[:<:]]", "", $data);
                            $temp = str_replace("[[:>:]]", "", $temp);
                            $temp = trim($temp);
                            $db->select( 'sKey', 'spdb_language', array( 'oType' => 'field_option', 'fid' => $this->fid, 'sValue' => "%{$temp}" ) );
                            $fids = $db->loadResultArray();
                            if(!count($fids)) {
                                $db->select( 'sKey', 'spdb_language', array( 'oType' => 'field_option', 'fid' => $this->fid, 'sKey' => "%{$temp}" ) );
                                $fids = $db->loadResultArray();
                            }
                        }
			if( count( $fids ) ) {
				foreach ( $fids as $opt ) {
					$db->dselect( 'sid', 'spdb_field_option_selected', array( 'copy' => '0', 'fid' => $this->fid, 'optValue' => $opt ), 'sid DESC' );
