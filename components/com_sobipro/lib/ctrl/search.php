...
final class SPSearchCtrl extends SPSectionCtrl {
	...
	if (count($this->_fields)) {
		...
		if (
		    ...
		    /* otherwise intersect these two arrays */ else {
			if (is_array($fr)) {
		...
		/* otherwise THESE are the results */ else {
...
//		exit;
	...
	$searchUserId = SPRequest::string('search_user_id', '');
	$searchFor = $this->_request['search_for'];
	$url = array('task' => 'search.results', 'sid' => Sobi::Section(), 'search_user_id' => $searchUserId, 'searchFor' => $searchFor);
	...
	if ($this->_task == 'results' && $ssid) {
	    ...
	    $url = array('task' => 'search.results', 'sid' => SPRequest::sid(), 'search_user_id' => $search_user_id, 'searchFor' => $searchFor);
	    if (!( SPRequest::cmd('ssid', null, 'cookie') )) {
		$url['ssid'] = $ssid;
	    }
