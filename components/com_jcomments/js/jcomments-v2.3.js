...
JComments.prototype = {
	...
	setForm: function(f){this.form=f;this.form_id=f.id;this.setMode('add',null);var jc=this;this.form.submit=function(){jc.saveComment();};this.form.keypressed=function(e){if(e.keyCode==27&&jc.mode=='reply'){jc.restoreForm(false);}};this.formLoaded();}, //if (typeof(js)!='undefined')
	...
	cancelEdit: function(i){this.$('dcs_img_list').innerHTML="";if((!this.cache[i])||(this.cache[i]=='')){this.$('comment-body-'+i).innerHTML=this.cache[i];this.cache[i]='';}if(this.form){this.form.remove('comment-id-hidden-'+i);this.restoreForm(true);}var t=this.$('comment-toolbar-'+i);if(t){t.style.display='';}return this.ajax('JCommentsCancelComment',arguments);},
	...
	updateList: function(t,m){
		this.restoreForm(true);//kiennd
		if(this.list_id){var l=this.$(this.list_id);if(!l){l=this.$('comments');m='a';}switch(m){case 'a':l.innerHTML=l.innerHTML+t;break;case 'p':l.innerHTML=t+l.innerHTML;break;case 'r':l.parentNode.innerHTML=t;break;}}},
	...
	setupPhotos: function(id,photos) {
		var jc=this;
		if(this.form==null){window.setTimeout(function(){jc.setupPhotos(id,photos);},500);return;}
		dcsSetupPhotos(photos);
	},
	...
	uploadImage: function(id){
		(function($){
			...
			$('form#comments-form').ajaxSubmit({
				success: function(data) {
					var jtajax = new jtAJAX();
					jtajax.processResponse(data);
				}
			});
		
		})(jQuery);		 
		this.$('dcs_img_list').innerHTML=""; // clear list
	},
	test: function(data) {
		alert(data);
	}
