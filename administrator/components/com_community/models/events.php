...
class CommunityModelEvents extends JModel
{
	...
	public function _buildQuery()
	{
		...
		$query		= 'SELECT a.* FROM ' . $db->nameQuote( '#__community_events' ) . ' AS a '
					. 'WHERE 1'
					. $condition
					. ' AND (( a.' . $db->nameQuote('repeat') .'=' . $db->quote('') .' || a.' . $db->nameQuote('repeat') .' IS NULL)' . ' OR (a.' . $db->nameQuote('parent') . '!= ' . $db->quote('0') .' && a.' . $db->nameQuote('published') .' != ' . $db->quote(3) . ' && a.' . $db->nameQuote('repeat') . ' IS NOT NULL)) '
					. $orderBy;
