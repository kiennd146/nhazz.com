...
// 		}
	...
	private function _updateUserNotiication( $key , $value )
	{
		...
		$query	= 'UPDATE ' . $db->nameQuote('#__community_users') . ' SET params=replace(params,' . $db->Quote( $left_exp.'1') . ',' . $db->Quote( $left_exp. $right_exp ) . ')';
		$db->setQuery( $query );
		$db->Query( );
		$query	= 'UPDATE ' . $db->nameQuote('#__community_users') . ' SET params=replace(params,' . $db->Quote( $left_exp.$quote.'1'.$quote) . ',' . $db->Quote( $left_exp. $right_exp ) . ')';
		$db->setQuery( $query );
		$db->Query( );
	}
