    ...
    public function __construct() {
        ...
        $head->addJsCode('
			window.addEvent( "domready", function() {
					...
					function SPValidate()
					{
                                        var $temp=parseInt($( "jform_params_limitChaDesc" ).value);
                                        var $cond=$( "sptpl" ).options[ $( "sptpl" ).selectedIndex ].value;
                                        var $cond1=$("jform_params_userid").value;
						if( $( "sid" ).value == 0 || $( "sid" ).value == "" || (!$temp && $temp!=0) || ($cond=="project.xsl" && $cond1=="") ) {
                                                    if(!$temp && $temp!=0){
                                                    alert("' . JText::_('JS_YOU_HAVE_TO_SELECT_NUMBER') . '" );
                                                    }
                                                    if($cond=="project.xsl" && $cond1==""){
                                                        alert("' . JText::_('JS_YOU_HAVE_TO_FILL_USERS_ID') . '" );
                                                     }
					...
					$( "spsection" ).addEvent( "change", function( event ) {
						...
						semaphor = 0;
                                        ...
                                        $( "sptpl" ).addEvent( "change", function( event ) {
						tpl = $( "sptpl" ).options[ $( "sptpl" ).selectedIndex ].value;
						if(tpl=="project.xsl"){
                                                    $("jform_params_userid").readOnly=false;
                                                } else {
                                                    $("jform_params_userid").readOnly="readonly";
                                                }
					} );
					if( $( "sp_category" ) != null ) {
...
//			'position.desc' => Sobi::Txt( 'SEC.CFG.ENTRY_ORDER_BY_POSITION_DESCENDING' ),
    ...
    private function tplFile($selected = null) {
        if (
                ...
                ( count(scandir(SOBI_PATH . '/usr/templates/front/modules/entries/')) < 5
                )) {
            $this->installTpl();
        }
        ...
        return SPHtml_Input::select('params[tplFile]', $tpls, $selected, false, $params);
    }
