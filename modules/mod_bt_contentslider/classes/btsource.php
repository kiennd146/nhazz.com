...
abstract class BTSource {
	...
	public function parseImages($row) {
		...
		if (!$this->_params->get('check_image_exist',1)) {
			...
			preg_match_all($regex, $text, $matches);
			    ...
			    //echo "start preg match" . time() . "<br/>";
                            preg_match_all($regex, $fulltext, $matches);
			    //echo "end preg match" . time() . "<br/>";
                        }
                        //echo "start foreach preg match" . time() . "<br/>";
			foreach ($matches[1] as $key => $match) {
				//echo "start get image size" . time() . "<br/>";
				@$url = getimagesize($match);
				//echo "end get image size" . time() . "<br/>";
				if (is_array($url)) {
			...
			//echo "end foreach preg match" . time() . "<br/>";
		}
	...
	public function generateImages($item, $isThumb = true) {
		//echo "Start parseImages : " . time() . "<br/>" ;
		$item = $this->parseImages($item);
		//echo "End parseImages : " . time() . "<br/>" ;
		//echo "Start create thumb : " . time() . "<br/>" ;
