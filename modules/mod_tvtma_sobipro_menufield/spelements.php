			...
			window.addEvent( "domready", function() {
					...
					function SPValidate()
					{
						if( $( "sid" ).value == 0 || $( "sid" ).value == "" ) {
							alert( "'.JText::_( 'JS_YOU_HAVE_TO_AT_LEAST_SELECT_A_SECTION' ).'" );
							return false;
						}
						else {
							return true;
					...
					$( "spsection" ).addEvent( "change", function( event ) {
						...
						semaphor = 0;
                                        ...
                                        $( "spfield" ).addEvent( "change", function( event ) {
						fid = $( "spfield" ).options[ $( "spfield" ).selectedIndex ].value;
						$( "fid" ).value = fid;
						semaphor = 0;
					} );
					if( $( "sp_category" ) != null ) {
...
//			'position.desc' => Sobi::Txt( 'SEC.CFG.ENTRY_ORDER_BY_POSITION_DESCENDING' ),
		...
		switch ( $label ) {
			...
			case 'tplFile':
				return JText::_( 'SOBI_TPL_FILE' );
                        case 'listfield':
				return JText::_( 'SOBI_LIST_FIELD' );
                        case 'fid':
				return JText::_( 'SOBI_SELECTED_FIELD' );
		}
	}
	...
	{
		if(
			!( file_exists( SOBI_PATH.'/usr/templates/front/modules/cat/' ) ) ||
			( count( scandir( SOBI_PATH.'/usr/templates/front/modules/cat/' ) ) < 5 )
		) {
			$this->installTpl();
		}
		$files = scandir( SOBI_PATH.'/usr/templates/front/modules/cat/' );
		$tpls = array();
		if( count( $files ) ) {
	...
	{
		SPFs::mkDir( SOBI_PATH.'/usr/templates/front/modules/cat/' );
		SPFactory::Instance( 'base.fs.directory', dirname( __FILE__ ).'/tmpl/' )
			->moveFiles( SOBI_PATH.'/usr/templates/front/modules/cat' );
	}
	...
	private function settings()
	{
		...
		return $settings;var_dump($settings);die;
	}
	...
	public function fetchElement( $name, &$label )
	{
		if( $name == 'sid'  ) {
			...
			return SPHtml_Input::text( 'params[sid]', $this->settings()->get( 'sid' ), $params );
                ...
                if( $name == 'fid'  ) {
			$params = array( 'id' => 'fid', 'size' => 5, 'class' => 'text_area', 'style' => 'text-align: center;', 'readonly' => 'readonly' );
			return SPHtml_Input::text( 'params[fid]', $this->settings()->get( 'fid' ), $params );
		}
		if( $name == 'spOrder' ) {
		...
		if( $name == 'tplFile' ) {
			return $this->tplFile( $this->settings()->get( 'tplFile' ) );
                ...
                if($name == 'listfield') {
                        return $this->Listfield($this->settings()->get('listfield'));
                }
        ...
        private function Listfield( $selected = NULL )
	{
            $sidd=$this->settings()->get( 'sid' );
            $db = JFactory::getDBO();
        $query = $db->getQuery(true);
        $query->select('fid,nid');
        $query->from('#__sobipro_field');
        $query->where("fieldType='select' AND section='".$sidd."'");
        $db->setQuery((string)$query);
        $messages = $db->loadObjectList();
        $fout=array();
        $fout[] = Sobi::Txt( 'SELECT_FIELD' );
        if( count( $messages ) ) {
			SPLoader::loadClass( 'models.datamodel' );
			SPLoader::loadClass( 'models.dbobject' );
			SPLoader::loadModel( 'field' );
			foreach ( $messages as $message ) {
				if( Sobi::Can( 'field', 'access', 'valid', $message->id ) ) {
					$f = new SPField();
					$f->extend( $message );
					$fout[ $f->get( 'id' ) ] = $this->getNameOfField($message->fid,'field','name');
				}
			}
                        
		
		}
       $params = array( 'id' => 'spfield');
       return SPHtml_Input::select(  'params[listfield]', $fout, $this->settings()->get( 'listfield' ), false, $params);
	}
        public function getNameOfField($field_key,$field_oType,$field_sKey)
        {
            $db = JFactory::getDBO();
            $query = $db->getQuery(true);
            $query->select('sValue ');
            $query->from('#__sobipro_language');
            $query->where("oType='".$field_oType."' AND fid='".$field_key."' AND sKey='".$field_sKey."'");
            $db->setQuery((string)$query);
            $field = $db->loadObject();
            return $field->sValue;
        }
