...
function drilldownmenu(setting){
	this.sublevelarrow={src:'modules/mod_tvtma_sobipro_menufield/includes/css/right.gif', width:'8px', top:'3px', left:'6px'} //full URL to image used to indicate there's a sub level
	this.breadcrumbarrow='modules/mod_tvtma_sobipro_menufield/includes/css/right.gif' //full URL to image separating breadcrumb links (if it's defined)
	this.loadingimage='modules/mod_tvtma_sobipro_menufield/includes/css/loader.gif' //full URL to 'loading' image, if menu is loaded via Ajax
	...
	this.backarrow='modules/mod_tvtma_sobipro_menufield/includes/css/leftarrow.gif' //full URL to image added in front of back LI 
...
drilldownmenu.prototype.buildmenu=function($){
	...
	this.$uls.each(function(i){ //loop through each UL
		...
		if (i==0){ //if topmost UL
			$('').prependTo($thisul).click(function(e){e.preventDefault()})
			...
			$('<li class="backcontrol"><img src="'+thisdrill.backarrow+'" style="border-width:0;vertical-align:middle !important;" />Quay lại trước</li>') //add back LI item
