...
$value = JRequest::getVar('searchFor');
...
if($type == 'list'):
	...
	$class = ($list->value == $value) ? "active" : "";
	$html .= "<li class='{$class}'>";
	$html .= JHTML::link(JRoute::_('index.php?option=com_sobipro&task=search.search&sp_search_for=' . $list->value . '&' . $fieldId . '&sid=' . $sectionId . '&spsearchphrase=exact'), $list->text);
	$html .=  "</li>";
    }
    $html .= "</ul>";
endif;
...
if($type == 'dropbox'):
    ...
    foreach ($lists as $list) {
	$class = ($list->value == $value) ? "selected='selected'" : "";
