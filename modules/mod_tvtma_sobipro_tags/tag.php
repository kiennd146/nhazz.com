        ...
        if ($messages)
        {
                foreach($messages as $message) 
                {
                    //$options[] = JHtml::_('select.option', $message->fid, $this->getNameOfField($message->fid));
										$select = ($message->fid == $this->value) ? "selected='selected'" : "";
                    $options .= "<option value='{$message->fid}' {$select} class='mod_tvtma_sobipro_option mod_tvtma_sobipro_sec_{$message->section}' >" . $this->getNameOfField('sValue',$message->fid, 'name', 'field') . "</option>";
                }
        }
