...
class BtContentDataSource extends BTSource {
	function getList() {
		...
		foreach ($items as &$item) {
			...
			$item->thumbnail = '';
			//echo "Start generateImages: " . time() . "<br/>" ;
			if ($params->get('show_image', 1)) {
				$item = $this->generateImages($item, $isThumb);
			}
			//echo "End generateImages: " . time() . "<br/>" ;
			...
			$item->authorLink = JRoute::_("index.php?option=com_community&view=profile&userid=" . $item->created_by);
			//echo "End Items: " . time() . "<br/><hr/>" ;
		}
		return $items;
	}
}
